document.addEventListener("DOMContentLoaded", () => {
  ...
  toImage.addEventListener("change", (event) => {
    ...
    // URL.createObjectURL() 또는 webkitURL.createObjectURL() 함수 중에
    // 하나를 사용하여 ...
   ...
   *
   * 하지만 그러기에는 event 코드가 너무 복잡해 질 수 있다.
   ...
   * 3-3. li tag 에는 data-seq 속성이 설정되어 있으므로
   *   자신의 dataset.seq 값을 getter 하면 된다.
  ...
  memo_box.addEventListener("click", async (e) => {
    ...
    // ul.memo 가 감싸고 있는 ul, span, img 태그중 "memo" 라는 클래스가 부착되어
    // 있으면
    if (classList.contains("memo")) {
      ...
      if (classList.contains("list")) {
        ...
        // span, img 를 클릭해도 li tag에 seq 값을 가져옴
  ...
  btn_save.addEventListener("click", () => {
     ...
     */
    // input_form 에 있는 저장버튼을 클릭했을때 toDate, toTime 의 정보를 저장
